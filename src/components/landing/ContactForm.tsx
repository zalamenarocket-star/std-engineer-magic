...
const ContactForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (elapsed < 3000) {
      toast({ title: t("Envio muito rápido", "Submission too fast"), description: t("Aguarde alguns segundos.", "Please wait a few seconds."), variant: "destructive" });
      return;
    }
    ...
    if (!emailRegex.test(formData.email.trim())) {
      toast({ title: t("E-mail inválido", "Invalid email"), description: t("Verifique o e-mail informado.", "Check the email provided."), variant: "destructive" });
      return;
    }
    const phoneClean = formData.phone.replace(/\D/g, "");
    if (phoneClean.length < 10 || phoneClean.length > 13) {
      toast({ title: t("Telefone inválido", "Invalid phone"), description: t("Informe um telefone válido.", "Enter a valid phone number."), variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      toast({ title: t("Formulário enviado com sucesso!", "Form submitted successfully!"), description: t("Entraremos em contato em breve.", "We'll get in touch soon.") });
      setFormData({ name: "", company: "", email: "", phone: "", service: "", message: "" });
      formLoadTime.current = Date.now();
    } catch {
      toast({ title: t("Erro ao enviar", "Error sending"), description: t("Tente novamente ou entre em contato pelo WhatsApp.", "Try again or contact us via WhatsApp."), variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
    ...
    <section id="formulario" className="py-20 bg-navy">
      <div className="container max-w-2xl">
        ...
        >
          ...
          <Input placeholder={t("Nome Completo *", "Full Name *")} required value={formData.name} onChange={update("name")} maxLength={100} className="bg-navy border-secondary/30 text-primary-foreground placeholder:text-primary-foreground/40" />
          <Input placeholder={t("Empresa", "Company")} value={formData.company} onChange={update("company")} maxLength={100} className="bg-navy border-secondary/30 text-primary-foreground placeholder:text-primary-foreground/40" />
          <div className="grid sm:grid-cols-2 gap-4">
            <Input type="email" placeholder={t("E-mail Corporativo *", "Corporate Email *")} required value={formData.email} onChange={update("email")} maxLength={255} className="bg-navy border-secondary/30 text-primary-foreground placeholder:text-primary-foreground/40" />
            <Input placeholder={t("Telefone/WhatsApp *", "Phone/WhatsApp *")} required value={formData.phone} onChange={update("phone")} maxLength={20} className="bg-navy border-secondary/30 text-primary-foreground placeholder:text-primary-foreground/40" />
          </div>
          <select
          ...
          <Textarea placeholder={t("Mensagem", "Message")} value={formData.message} onChange={update("message")} maxLength={1000} className="bg-navy border-secondary/30 text-primary-foreground placeholder:text-primary-foreground/40 min-h-[100px]" />
          <Button type="submit" size="lg" className="w-full font-heading font-bold uppercase tracking-wide text-base" disabled={loading}>
            {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> {t("Enviando...", "Sending...")}</> : t("Fale com um Engenheiro", "Talk to an Engineer")}
          </Button>
